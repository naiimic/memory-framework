...
def retrieve(
    ...
    documents: List[str],
    num_memories_queried: int,
) -> List[str]:
    """Retrieve the num_memories_queried most similar documents for a given query."""

    if len(doc_embeddings) != len(documents):
        raise ValueError("Mismatched lengths of document embeddings and documents")

    assert num_memories_queried > 0, "Must retrieve at least one document"
    ...
    top_k_indices = similarities.argsort()[-num_memories_queried:][::-1]
